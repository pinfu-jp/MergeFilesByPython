...
class TestMergeFilesByPython(unittest.TestCase):
    def test_extract_timestamp_and_write_to_csv(self):
        ...
        with open('test_output.csv', 'r') as f:
            lines = f.readlines()
            self.assertEqual(lines[0], 'timestamp,log_message\n')
            self.assertEqual(lines[1], '2022/12/31 14:51:11,log message1\n')
            self.assertEqual(lines[2], '2022/12/31 14:51:12,log message2\n')
